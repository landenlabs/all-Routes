...
import static landenlabs.routes.data.ArrayListEx.get;
import static landenlabs.routes.data.GpsPoint.GPS_TIME_FMT;
import static landenlabs.routes.data.GpsPoint.NO_POINT;
import static landenlabs.routes.logger.Analytics.Event.PAGE_WEATHER;
import static landenlabs.routes.map.MapViewer.BIG_ICON_SCALE;
import static landenlabs.routes.pages.PagesSmartAux.SmartAlert.DRIVE_SPEED_KPH;
...
import landenlabs.routes.databinding.PageWeatherFragBinding;
...
import landenlabs.routes.pages.PagesSmartAux.SmartAdapter;
import landenlabs.routes.pages.PagesSmartAux.SmartAlert;
import landenlabs.routes.pages.PagesSmartAux.SmartAlertLoader;
import landenlabs.routes.pages.PagesSmartAux.SmartItemHolder;
import landenlabs.routes.pages.PagesSmartAux.SmartViewHelper;
import landenlabs.routes.utils.FmtTime;
...
public class PageWeatherFrag extends PageBaseFrag implements
    ...
    public void onLongTouch(MapViewer mapViewer, TWCMapTouch touch) {
        clearAll();
        WLatLng latLng = GpsUtils.toWLatLng(touch.getGeoPoint());
            ...
            if (smartAlerts.size() > 2) {
                ...
                try {
                    ...
                    for (SmartAlert smartAlert : smartAlerts) {
                        ...
                        track.add(pt);
                        markers.addMarker(pt.toLatLng(), "Pt" + idx++, smartAlert.getImageRes(requireContext()), 2f);
                    }
                    return;
                } catch (Exception ex) {
    ...
    public void onMapReady(MapViewer mapViewer, int why) {
        if (why == MAP_STATE_READY) {
            // "Radar" - see com.weather.mapsdk.enums.TWCMapSDKRasterLayerType
            String rasterName = "Radar";
            if (!mapViewer.setRasterLayer(rasterName))
                ALog.w.tagMsg(mapViewer.getTag(), "Failed to set map raster layer " + rasterName);
